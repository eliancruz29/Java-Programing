...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		...
		if (lastSelected == null) {
			for (Marker item : markers) {
				if (item.isInside(map, mouseX, mouseY)) {
					lastSelected = (CommonMarker) item;
	...
	public void mouseClicked() {
		...
		if (lastClicked != null) {
			...
			Marker marker = anyMarkerIsSelected();
			if(marker != null){
	...
	private void hideOtherMarkers(Marker item){
		if(item instanceof EarthquakeMarker){
			EarthquakeMarker earthquake = (EarthquakeMarker)item;
			double dist = earthquake.threatCircle();			
			for(Marker marker : cityMarkers){
				if((dist >= earthquake.getDistanceTo(marker.getLocation()))){
			...
			CityMarker city = (CityMarker)item;
			for(Marker marker : quakeMarkers){				
				if(((EarthquakeMarker)marker).threatCircle() >= city.getDistanceTo(marker.getLocation())){
					marker.setHidden(false);
					marker.setSelected(false);
				}else{
					marker.setHidden(true);
	...
	// this method verify is a marker was selected
	private Marker anyMarkerIsSelected() {
		// make a loop to verify if a earthquake is in the position of the click
		for (Marker marker : quakeMarkers) {
			if (marker.isInside(map, mouseX, mouseY)) {
				// update this earthquake to selected
				marker.setSelected(true);
				return marker;
			}
		}
		// make a loop to verify if a city is in the position of the click
		for (Marker marker : cityMarkers) {
			if (marker.isInside(map, mouseX, mouseY)) {
				// update this city to selected
				marker.setSelected(true);
				return marker;
			}
		}
